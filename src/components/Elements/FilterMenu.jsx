...
import FilterMultiSelectObjectDetail from './FilterMultiSelectObjectDetail'; // Поле для выбора нескольких значений из списка (Модифицированная версия с передачей id)
            ...
            <div className="filter-items">
                {filters.map((filter, index) => (
                    <div key={index} className="filter-item">
                        ...
                        {filter.type === 'multi-select-extended' && (
                            <FilterMultiSelectObjectDetail
                                placeholder={filter.placeholder || ''}
                                options={filter.options}
                                selectedValues={formData[filter.name] || []}
                                onChange={(values) => handleMultiSelectChange(values, filter.name)}
...
FilterMenu.propTypes = {
    ...
    filters: PropTypes.arrayOf(
        PropTypes.shape({
            type: PropTypes.oneOf(['text', 'number', 'date-range', 'date-range-no-time', 'select', 'multi-select', 'multi-select-extended']).isRequired,
